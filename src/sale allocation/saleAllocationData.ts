...
export interface SaleAllocationLineDocument {
  productCode: string;
  productName: string;
  category: string;
  uom: string;
...
export interface SaleAllocationDocument {
  id: string;
  number: string;
  ...
  instructions: string;
  totalAmount: string;
  lines: SaleAllocationLineDocument[];
}

export const saleAllocationDocuments: SaleAllocationDocument[] = [
  {
    id: 'sa-1001',
    number: 'SA-2026-00011',
...
export const extendedSaleAllocationDocuments: SaleAllocationDocument[] = Array.from(
  { length: 2 },
  (_, cycle) =>
    saleAllocationDocuments.map((document, index) => {
      const sequence = cycle * saleAllocationDocuments.length + index + 1;
      const paddedSequence = String(sequence + 10).padStart(5, '0');

      return {
        ...document,
        id: `${document.id}-${sequence}`,
        number: `SA-2026-${paddedSequence}`,
      };
    })
).flat();

export function getSaleAllocationById(
  id: string | null
): SaleAllocationDocument | undefined {
  if (!id) {
    return undefined;
  }

  return extendedSaleAllocationDocuments.find((document) => document.id === id);
}

export type SaleAllocationRequisitionLineDocument = SaleAllocationLineDocument;
export type SaleAllocationRequisitionDocument = SaleAllocationDocument;
export const saleAllocationRequisitionDocuments = saleAllocationDocuments;
export const extendedSaleAllocationRequisitionDocuments = extendedSaleAllocationDocuments;
export const getSaleAllocationRequisitionById = getSaleAllocationById;
