...
import type { SaleAllocationDocument } from './saleAllocationData';
import {
...
interface CreateSaleAllocationProps {
  editingDocument?: SaleAllocationDocument;
  onBack: () => void;
  onNavigateToSaleAllocationList: () => void;
}

type SearchMode = 'sale-order' | 'sale-requisition' | 'customer';
...
function createAllocationLinesFromSaleRequisition(
  document: SaleAllocationRequisitionDocument
): SaleAllocationLineForm[] {
  return document.lines.flatMap((line, index) => {
    ...
    if (itemType !== 'Serialized') {
      const lineForm: SaleAllocationLineForm = {
        id: `sa-sar-line-${document.id}-${index}`,
        itemType,
        productCode: line.productCode,
        productName: line.productName,
    ...
    const requestedQuantity = Math.max(0, Math.floor(parseDecimal(line.requestedQty)));
    const allocatedQuantity = Math.floor(parseDecimal(line.allocatedQty));

    return Array.from({ length: requestedQuantity }, (_, itemIndex): SaleAllocationLineForm => ({
      id: `sa-sar-line-${document.id}-${index}-${itemIndex + 1}`,
      itemType: 'Serialized',
      productCode: line.productCode,
      productName: line.productName,
      hsnSac: product?.hsnSac ?? '',
      ...
      serialNumber: '',
      requirementDate: line.requirementDate,
      requestedQty: formatDecimal(1),
      allocatedQty: itemIndex + 1 <= allocatedQuantity ? formatDecimal(1) : '',
      allocationQty: '',
      location: '',
      secondarySerialNumber: '',
      requestedAllocationDate: line.requirementDate,
...
function mapDocumentToForm(document?: SaleAllocationDocument): SaleAllocationFormData {
  if (!document) {
    return {
...
function mapDocumentLines(document?: SaleAllocationDocument): SaleAllocationLineForm[] {
  if (!document) {
...
const CreateSaleAllocation: React.FC<CreateSaleAllocationProps> = ({
  editingDocument,
  onBack,
  ...
  const [selectedSaleRequisitionId, setSelectedSaleRequisitionId] = useState<string | null>(null);
  ...
  const matchingSaleRequisitions = useMemo(() => {
    const normalizedSearch = searchQuery.trim().toLowerCase();
    if (!normalizedSearch) {
      return extendedSaleAllocationRequisitionDocuments;
    }

    return extendedSaleAllocationRequisitionDocuments.filter(
      (document) =>
        document.number.toLowerCase().includes(normalizedSearch) ||
        document.customerName.toLowerCase().includes(normalizedSearch) ||
        document.salesExecutive.toLowerCase().includes(normalizedSearch) ||
  ...
  const selectedSaleRequisition = useMemo(
    () =>
      extendedSaleAllocationRequisitionDocuments.find(
        (document) => document.id === selectedSaleRequisitionId
      ) ?? null,
    [selectedSaleRequisitionId]
  );
  ...
  const canManuallyAddProductLine = Boolean(selectedCustomer && !selectedSaleOrder && !selectedSaleRequisition);
  ...
  const handleSelectSaleOrder = (saleOrderId: string) => {
    ...
    setSelectedSaleRequisitionId(null);
  ...
  const handleSelectSaleRequisition = (documentId: string) => {
    const document = extendedSaleAllocationRequisitionDocuments.find((option) => option.id === documentId);
    if (!document) {
      return;
    }

    const linkedCustomer =
      customerOptions.find((option) => option.value === document.customerName) ??
      customerOptions.find((option) => option.label === document.customerName) ??
      null;

    setSelectedSaleRequisitionId(document.id);
    ...
    const nextLines = createAllocationLinesFromSaleRequisition(document);
    setLines(nextLines);
    setActiveProductLineTab(
      nextLines.some((line) => line.itemType === 'Serialized') ? 'Serialized' : 'Non-serialized'
    );
  ...
  const clearSearchSelection = () => {
    if (searchMode === 'sale-order' || searchMode === 'sale-requisition') {
      setSelectedSaleOrderId(null);
      setSelectedSaleRequisitionId(null);
    ...
    >
        ...
        <div className="create-pr-header__top">
          <div className="create-pr-header__title-group">
            <a
              ...
              aria-label="Back to sale allocation list"
            >
              <ArrowLeft size={18} />
            </a>
            <div className="create-pr-header__title-wrap">
              <div className="create-pr-header__title-row">
                <h2 className="brand-page-title create-pr-header__title">
                  {editingDocument ? 'Edit Sale Allocation' : 'New Sale Allocation'}
                </h2>
          ...
          >
            <div className="po-create__requisition-search">
              <div className="po-create__requisition-search-shell sale-allocation-create__search-shell">
                ...
                {(searchMode === 'sale-order'
                  ? selectedSaleOrder
                  : searchMode === 'sale-requisition'
                    ? selectedSaleRequisition
                    : selectedCustomer) && (
                  <span className="po-create__selected-chip">
                    <span className="po-create__selected-chip-text">
                      {searchMode === 'sale-order'
                        ...
                        : searchMode === 'sale-requisition'
                          ? selectedSaleRequisition?.number
                ...
                <input
                  ...
                  placeholder={
                    (searchMode === 'sale-order'
                      ...
                      : searchMode === 'sale-requisition'
                        ? 'Search sale requisition by number, customer, executive...'
            ...
            {isSearchResultsOpen &&
              (searchMode === 'sale-order'
                ...
                : searchMode === 'sale-requisition'
                  ? matchingSaleRequisitions.length > 0
                  ...
                  <table className="po-create__requisition-results-table">
                    ...
                    <tbody>
                      {searchMode === 'sale-order'
                        ...
                        : searchMode === 'sale-requisition'
                          ? matchingSaleRequisitions.map((document: SaleAllocationRequisitionDocument) => (
                              <tr
                                key={document.id}
                                className="po-create__requisition-results-row"
                                role="option"
                                tabIndex={0}
                                onMouseDown={(event) => event.preventDefault()}
                                onClick={() => handleSelectSaleRequisition(document.id)}
                                onKeyDown={(event) => {
                                  if (event.key === 'Enter' || event.key === ' ') {
                                    event.preventDefault();
                                    handleSelectSaleRequisition(document.id);
                                  }
                                }}
                              >
                                <td className="po-create__requisition-results-number">
                                ...
                                <td>{document.salesExecutive}</td>
                                <td>{formatDate(document.requestedDeliveryDate)}</td>
      ...
      <div className="create-pr-page mx-auto w-full max-w-[1800px] px-6 py-6 space-y-6">
        <div className="create-pr-tabs">
          <div className="create-pr-tabs__list" role="tablist" aria-label="Sale allocation sections">
            {tabs.map((tab) => (
              <button
                key={tab.id}
                type="button"
                  ...
                  <thead>
                    {activeProductLineTab === 'Serialized' ? (
                      <tr>
                        ...
                        <th className="create-pr-grid__cell create-pr-grid__cell--number">Allocation Req. Qty</th>
                        ...
                        <tr key={line.id}>
                          ...
                          {activeProductLineTab === 'Serialized' ? (
                            <>
                              ...
                              <td className="create-pr-grid__body-cell create-pr-grid__body-cell--number"><Input value={formatReadOnlyQuantity(line.requestedQty)} readOnly disabled className="create-pr-grid__control create-pr-grid__control--readonly create-pr-grid__control--number min-w-28" /></td>
                    ...
                    {filteredProductLines.length === 0 && (
                      <tr>
                        <td
                          ...
                          colSpan={activeProductLineTab === 'Serialized' ? 14 : 13}
                        >
      ...
      <SuccessSummaryDialog
        ...
        documentLabel="Sale allocation No"
