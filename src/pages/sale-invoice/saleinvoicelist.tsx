...
} from './saleInvoiceData';

interface SaleInvoiceListProps {
  onNew: () => void;
...
const SaleInvoiceList: React.FC<SaleInvoiceListProps> = ({
  onNew,
  onEdit,
  onNavigateToSaleInvoiceList,
}) => {
  const [documents, setDocuments] = useState<SaleInvoiceDocument[]>(extendedSaleInvoiceDocuments);
  ...
  const filteredRows = useMemo(() => {
    ...
    return documents.filter((item) => {
      ...
      const matchesExecutive = !filters.branch || item.salesExecutive === filters.branch;
      const invoiceDate = item.invoiceDateTime.slice(0, 10);
      const matchesStartDate = !filters.startDate || invoiceDate >= filters.startDate;
      const matchesEndDate = !filters.endDate || invoiceDate <= filters.endDate;
      const matchesFilterSet =
        matchesCustomer && matchesPriority && matchesExecutive && matchesStartDate && matchesEndDate;
    ...
    return [...insightFilteredRows].sort((left, right) => {
      ...
      switch (sortState.key) {
        ...
        case 'invoiceDateTime':
          comparison = new Date(left.invoiceDateTime).getTime() - new Date(right.invoiceDateTime).getTime();
          break;
        case 'saleOrderNumber':
          comparison = left.saleOrderNumber.localeCompare(right.saleOrderNumber, undefined, { numeric: true });
          break;
        case 'customerName':
  ...
  const insightItems = useMemo(() => {
    ...
    return [
      buildCountInsight({
        ...
        count: total,
        total,
        support: `${formatInsightCount(total)} sale invoices`,
        hint: `${formatInsightAmount(totalValue)} invoiced value`,
        tone: 'neutral',
      }),
      buildCountInsight({
        key: 'pending',
        ...
        count: pending,
        total,
        support: `${pending} invoices need approval`,
      ...
      buildCountInsight({
        ...
        count: approved,
        total,
        support: `${approved} invoices approved`,
  ...
  const renderActionMenu = (item: SaleInvoiceDocument) => (
    <div
  ...
  return (
    <AppShell activeLeaf="sale-invoice" onSaleInvoiceClick={onNavigateToSaleInvoiceList}>
          ...
          <div className="catalogue-toolbar__top">
            <div className="catalogue-toolbar__heading">
              <div className="catalogue-toolbar__title-row">
                <button type="button" className="catalogue-toolbar__title-button" aria-label="Sale invoice catalogue">
                  <h2 className="brand-page-title">My Sale Invoice</h2>
                </button>
              ...
              <div className="catalogue-toolbar__utility-group">
                <div className="catalogue-toolbar__search">
                  ...
                  <input
                    ...
                    aria-label="Search sale invoices"
                  />
                </div>

                <div className="catalogue-view-toggle" role="group" aria-label="Sale invoice view mode">
                  <button
                    type="button"
                ...
                <button
                  ...
                  aria-label={hasActiveFilters ? `Filter sale invoices. ${activeFilterCount} filters applied.` : 'Filter sale invoices'}
                >
                  <Filter size={16} />
                  Filters
      ...
      <div className="mx-auto flex w-full max-w-[1800px] flex-col gap-4 px-4 py-4">
        {loadState === 'ready' && (
          <CatalogueInsightCards
            ...
            ariaLabel="Sale invoice insights"
        ...
        {loadState === 'ready' && sortedRows.length === 0 && (
          <div className="flex flex-col items-center rounded border border-dashed border-slate-300 bg-slate-50 px-6 py-12 text-center">
            ...
            <h2 className="text-lg font-bold text-slate-900">No sale invoices found</h2>
            <p className="mt-2 max-w-xl text-sm text-slate-500">
              Adjust your filters or search a different invoice, customer, or sale order.
            </p>
        ...
        {loadState === 'ready' && sortedRows.length > 0 && catalogueViewMode === 'list' && (
          <div className="catalogue-table-scroll">
            <table className="catalogue-table">
              <thead>
                <tr>
                  <SortableTableHeader label="SI No." sortKey="number" sortState={sortState} onSortChange={handleSortChange} />
                  <SortableTableHeader label="Document date & time" sortKey="invoiceDateTime" sortState={sortState} onSortChange={handleSortChange} dataType="date" />
                  <SortableTableHeader label="SO No." sortKey="saleOrderNumber" sortState={sortState} onSortChange={handleSortChange} />
              ...
              <tbody>
                {sortedRows.map((item) => {
                  const documentDateTime = formatDateTime(item.invoiceDateTime);
                ...
                <article key={item.id} className="catalogue-card">
                  ...
                  <div className="catalogue-card__body">
                    <div className="catalogue-card__info-grid">
                      ...
                      <span className="catalogue-card__label">SO No.</span>
                      <span className="catalogue-card__value">{item.saleOrderNumber}</span>
                      <span className="catalogue-card__label">Sales executive</span>
                      <span className="catalogue-card__value">{item.salesExecutive}</span>
                      <span className="catalogue-card__label">Order date</span>
                      <span className="catalogue-card__value">{formatDate(item.orderDate)}</span>
      ...
      <CatalogueFilterDrawer
        ...
        subtitle="Narrow down sale invoices by customer, priority, executive, and invoice date."
        draftFilters={draftFilters}
        primaryLabel="Customer"
        primaryOptions={customerOptions}
        branchLabel="Sales Executive"
        branchOptions={executiveOptions}
        dateFromLabel="Invoice from"
        dateToLabel="Invoice to"
