...
import { extendedPurchaseOrderDocuments as extendedPurchaseReceiptDocuments, type PurchaseOrderDocument as PurchaseReceiptDocument } from './purchaseReceiptData';
...
const FilterDrawer: React.FC<{
  ...
  return (
    <SideDrawer
      isOpen={isOpen}
      title="Filters"
      subtitle="Refine purchase receipts using date, status, supplier, priority, and creator criteria."
      onClose={onClose}
    ...
    >
      <div className="drawer-form">
        ...
        <div className="drawer-form__field">
          ...
          <div className="drawer-multiselect" role="group" aria-label="Select purchase receipt status filters">
...
const PurchaseReceiptPreviewDrawer: React.FC<{
  document: PurchaseReceiptDocument | null;
  isOpen: boolean;
  onClose: () => void;
  onEdit: (document: PurchaseReceiptDocument) => void;
  onCancel: (document: PurchaseReceiptDocument) => void;
...
const PurchaseReceiptList: React.FC<PurchaseReceiptListProps> = ({
  ...
  const [documents, setDocuments] = useState<PurchaseReceiptDocument[]>(extendedPurchaseReceiptDocuments);
  ...
  const insightItems = useMemo(() => {
    ...
    return [
      buildCountInsight({
        ...
        count: total,
        total,
        support: `${formatInsightCount(total)} purchase receipts`,
        hint: `${formatInsightAmount(totalValue)} receipt value`,
        tone: 'neutral',
      }),
      buildCountInsight({
        key: 'pending',
        ...
        count: pending,
        total,
        support: `${pending} receipts need review`,
      ...
      buildCountInsight({
        ...
        count: approved,
        total,
        support: `${approved} receipts confirmed`,
        hint: `${getInsightPercent(approved, total)}% approved`,
        tone: 'success',
      }),
      buildCountInsight({
        key: 'high-value',
        label: 'High value receipts',
        count: highValue,
        total,
          ...
          <div className="catalogue-toolbar__top">
            <div className="catalogue-toolbar__heading">
              <div className="catalogue-toolbar__title-row">
                <button type="button" className="catalogue-toolbar__title-button" aria-label="Purchase receipt views">
                  <h2 className="brand-page-title">My Purchase Receipt</h2>
                  <ChevronDown size={16} />
                </button>
            ...
            <div className="catalogue-toolbar__actions">
              ...
              <div className="catalogue-toolbar__primary-group">
                <button
                  ...
                  aria-label={hasActiveFilters ? `Filter purchase receipts. ${activeFilterCount} filters applied.` : 'Filter purchase receipts'}
                >
                  <Filter size={16} />
                  Filters
      ...
      <div className="mx-auto flex w-full max-w-[1800px] flex-col gap-4 px-4 py-4">
        {loadState === 'ready' && (
          <CatalogueInsightCards
            ...
            ariaLabel="Purchase receipt insights"
        ...
        <section>
          ...
          {loadState === 'error' && (
            <div className="rounded border border-rose-200 bg-rose-50 p-6 text-sm text-rose-700">
              Purchase receipts could not be loaded right now.
          ...
          {loadState === 'ready' && sortedRows.length === 0 && (
            <div className="flex flex-col items-center rounded border border-dashed border-slate-300 bg-slate-50 px-6 py-12 text-center">
              ...
              <h2 className="text-lg font-bold text-slate-900">No purchase receipts found</h2>
