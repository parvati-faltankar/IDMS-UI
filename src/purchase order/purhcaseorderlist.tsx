...
const FilterDrawer: React.FC<{
  ...
  return (
    <SideDrawer
      isOpen={isOpen}
      title="Filters"
      subtitle="Refine purchase orders using date, status, supplier, priority, and creator criteria."
      onClose={onClose}
    ...
    >
      <div className="drawer-form">
        ...
        <div className="drawer-form__field">
          ...
          <div className="drawer-multiselect" role="group" aria-label="Select purchase order status filters">
...
const PurchaseOrderPreviewDrawer: React.FC<{
...
const PurhcaseOrderList: React.FC<PurhcaseOrderListProps> = ({
  filters,
  onFiltersChange,
  onNew,
  onEdit,
  ...
  const [documents, setDocuments] = useState<PurchaseOrderDocument[]>(extendedPurchaseOrderDocuments);
  ...
  const insightItems = useMemo(() => {
    ...
    return [
      buildCountInsight({
        ...
        count: total,
        total,
        support: `${formatInsightCount(total)} purchase orders`,
        hint: `${formatInsightAmount(totalValue)} total order value`,
        tone: 'neutral',
      }),
      buildCountInsight({
        key: 'pending',
        ...
        count: pending,
        total,
        support: `${pending} POs need approval`,
      ...
      buildCountInsight({
        ...
        count: approved,
        total,
        support: `${approved} POs ready for execution`,
          ...
          <div className="catalogue-toolbar__top">
            <div className="catalogue-toolbar__heading">
              <div className="catalogue-toolbar__title-row">
                <button type="button" className="catalogue-toolbar__title-button" aria-label="Purchase order views">
                  <h2 className="brand-page-title">My Purchase Order</h2>
                  <ChevronDown size={16} />
                </button>
            ...
            <div className="catalogue-toolbar__actions">
              ...
              <div className="catalogue-toolbar__primary-group">
                <button
                  ...
                  aria-label={hasActiveFilters ? `Filter purchase orders. ${activeFilterCount} filters applied.` : 'Filter purchase orders'}
                >
                  <Filter size={16} />
                  Filters
      ...
      <div className="mx-auto flex w-full max-w-[1800px] flex-col gap-4 px-4 py-4">
        {loadState === 'ready' && (
          <CatalogueInsightCards
            ...
            ariaLabel="Purchase order insights"
        ...
        <section>
          ...
          {loadState === 'error' && (
            <div className="rounded border border-rose-200 bg-rose-50 p-6 text-sm text-rose-700">
              Purchase orders could not be loaded right now.
          ...
          {loadState === 'ready' && sortedRows.length === 0 && (
            <div className="flex flex-col items-center rounded border border-dashed border-slate-300 bg-slate-50 px-6 py-12 text-center">
              ...
              <h2 className="text-lg font-bold text-slate-900">No purchase orders found</h2>
...
export default PurhcaseOrderList;
