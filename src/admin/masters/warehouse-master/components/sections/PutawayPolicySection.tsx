...
const RECOMMENDED_SEQUENCE: PutawayStrategy[] = [
  'FEFO',
  'Nearest-Empty',
  'Capacity-Optimised',
  'Fixed-BIN',
...
function defaultPolicy(): PutawayPolicy {
  return {
    enabled: true,
    strategy: 'FEFO',
    strategySequence: ['FEFO', 'Nearest-Empty', 'Capacity-Optimised'],
    overrideAllowed: true,
  };
}

function toLocal(warehouse: ConfigSectionProps['warehouse']): PutawayPolicy {
  const policy = warehouse.autoPutaway ?? defaultPolicy();
...
export function PutawayPolicySection({ warehouse, locations, readOnly, saving, onSave }: ConfigSectionProps) {
  const [local, setLocal] = useState<PutawayPolicy>(() => toLocal(warehouse));
  const [dirty, setDirty] = useState(false);
  const [pendingStrategy, setPendingStrategy] = useState<PutawayStrategy>('Fixed-BIN');
  const [showSimulation, setShowSimulation] = useState(false);

  const isBinLevel = warehouse.inventoryControlMode === 'Location-BIN-Level';

  const simulation = useMemo(
    () => simulatePutawayStrategy(local.strategySequence, locations),
  ...
  function set<K extends keyof PutawayPolicy>(key: K, value: PutawayPolicy[K]) {
    setLocal((current) => ({ ...current, [key]: value }));
    setDirty(true);
  }

  function updateSequence(nextSequence: PutawayStrategy[]) {
    setLocal((current) => ({
      ...current,
      ...
      <div style={sCard}>
        ...
        <div style={{ ...sBody, opacity: local.enabled ? 1 : 0.45 }}>
          <div style={{ marginBottom: '14px', padding: '12px 14px', border: '1px solid #BFDBFE', background: '#EFF6FF', borderRadius: '8px', fontSize: '12px', color: '#1D4ED8' }}>
            Auto Putaway filters candidate locations before sorting them by the ordered strategy list. The first
            strategy in the sequence becomes the primary strategy.
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '240px 1fr', gap: '14px', alignItems: 'end', marginBottom: '16px' }}>
            <div>
              ...
              <select
                ...
                onChange={(event) => setPendingStrategy(event.target.value as PutawayStrategy)}
          ...
          <div style={{ marginBottom: '16px' }}>
            ...
            <p style={{ ...hintTxt, marginBottom: '10px' }}>
              Ordered builder only. Candidate locations are filtered first, then sorted top-to-bottom by this sequence.
            </p>
            {local.strategySequence.length === 0 ? (
              <div style={{ padding: '18px', border: '1px dashed var(--color-border)', borderRadius: '8px', color: 'var(--color-text-muted)', fontSize: '12px' }}>
                Add at least one strategy to enable putaway automation.
              </div>
            ) : (
          ...
          <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer', fontSize: '12px' }}>
            <input
              ...
              style={{ accentColor: 'var(--color-primary)' }}
            />
            Allow receiving users to override the recommended destination
          </label>
        </div>
      </div>
        ...
        <div style={sCard}>
          ...
          <div style={sBody}>
            ...
            <p style={{ ...hintTxt, marginBottom: '12px' }}>
              Filters applied before sorting: Active leaf locations, inventory allowed, putaway not blocked, hard-capacity check.
            </p>
            <div style={{ display: 'grid', gap: '8px' }}>
              {simulation.candidates.length === 0 ? (
                <div style={{ padding: '16px', border: '1px dashed var(--color-border)', borderRadius: '8px', fontSize: '12px', color: 'var(--color-text-muted)' }}>
                  No eligible candidate locations found for preview.
                </div>
              ) : (
