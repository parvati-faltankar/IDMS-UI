...
Purpose: Base drawer shell providing standardised header (title, subtitle, status label), scrollable body with loading skeleton, footer with typed action buttons, and dirty-state close guard.

Rules:

- Wrap with SmartPreviewDrawer, SmartFormDrawer, or SmartReviewDrawer rather than using directly where a typed variant applies.
- Width tokens: `sm`=400px, `md`=520px, `lg`=680px, `xl`=840px. Default is `md`.
- Always provide a meaningful `title`. `subtitle` and `statusLabel` are optional enrichments.
- `footerActions` items require at minimum a `label` and `onClick`. Use `tone` of `primary`, `danger`, or `outline`.
- When `isDirty` is true, the × close button prompts before closing — do not bypass this guard.
- Anchors to the right edge of the viewport. Do not override the anchor or z-index.

## SmartPreviewDrawer

Purpose: Standardised read-only record preview drawer with a summary fields grid and expandable section cards. Used for glanceable record inspection without navigating away from the list.

Rules:

- Use on admin list pages where row click should show record details without full-page navigation.
- `summaryFields` (max 4 items) appear in a 2-column grid at the top — include Code, Status, and key dates.
- `sections` render as titled cards below the summary. Limit to 3–5 sections; keep field count per section to 6 or fewer.
- Provide a `primaryAction` (e.g. Edit) and optional `secondaryActions` and `dangerAction` (e.g. Deactivate).
- Do not put editable form controls inside SmartPreviewDrawer — use SmartFormDrawer for that.
- Width defaults to `lg`. Do not force a narrower width for record previews.

## SmartFormDrawer

Purpose: Drawer-hosted form shell with a Cancel / Save footer, validation error strip, loading state, and dirty-state close guard. For quick-create and quick-edit operations that do not require full-page forms.

Rules:

- Use only for forms with ≤8 fields. Larger or multi-section forms must remain full-page.
- `onSave` receives the submit event — call your API and close only on success.
- `validationErrors` (string[]) renders a red error strip above the form body when non-empty.
- `isDirty` must be true whenever the user has modified any field — triggers the close guard.
- `saveDisabled` must reflect the form's submittable state — do not allow saving an incomplete form.
- Width defaults to `md`. Use `sm` only for single-field quick-edit drawers.

## SmartReviewDrawer

Purpose: Activation / confirmation drawer that surfaces a checklist of requirements, a summary of key fields, and a consequence statement before a destructive or irreversible action.

Rules:

- Use before any activation, deactivation, or destructive bulk action that requires checklist verification.
- `checklistItems` drive both the visual checklist and whether Confirm is enabled — all must pass.
- `summaryFields` should restate the record's key identity fields (name, code, type) for confirmation.
- `consequenceNote` is required — describe what will happen after the user confirms.
- The Confirm button is disabled until every checklist item has `passed: true`.
- Width defaults to `lg`. Do not narrow below `md` — the checklist needs reading space.
